...
namespace ChampionsOfForest.Player
{
	public class CoopCustomWeapons : MonoBehaviour
	{
		...
		{
			public ItemTemplate.WeaponModelType model;
			...
			public CustomWeaponPrefabData(ItemTemplate.WeaponModelType model, Vector3 offset, Vector3 rotation, Vector3 tip, float scale, GameObject g)
			{
				this.model = model;
				this.offset = offset;
		...
		public static void Init()
		{
			...
			//longsword
			AddWeapon(ItemTemplate.WeaponModelType.LongSword,
				51,
				Core.CreateMaterial(
					new BuildingData()
					{
			...
			//great sword
			AddWeapon(ItemTemplate.WeaponModelType.GreatSword,
				52,
				Core.CreateMaterial(
					new BuildingData()
					{
			...
			//hammer
			AddWeapon(ItemTemplate.WeaponModelType.Hammer,
					 108,
					 Core.CreateMaterial(
						 new BuildingData()
						 {
			...
			AddWeapon(ItemTemplate.WeaponModelType.Axe, Instantiate(Res.ResourceLoader.GetAssetBundle(2001).LoadAsset<GameObject>("AxePrefab.prefab")),
				new Vector3(-0.05901787f, -0.075443f, -1.6f),
				new Vector3(0, 180, 0),
				new Vector3(0, 0, 0),
				1);
			AddWeapon(ItemTemplate.WeaponModelType.Greatbow, 167,
		...
		public static void AddWeapon(ItemTemplate.WeaponModelType model, int mesh, Material material, Vector3 offset, Vector3 rotation, Vector3 tip, float Scale)
		{
			try
			{
		...
		public static void AddWeapon(ItemTemplate.WeaponModelType model, GameObject go, Vector3 offset, Vector3 rotation, Vector3 tip, float Scale)
		{
			try
			{
				prefabs.Add((int)model, go);
		...
		private IEnumerator EquipDelayed(Transform handTransform, int i)
		{
			...
			yield return null;
			if (i != (int)ItemTemplate.WeaponModelType.None)
			{
