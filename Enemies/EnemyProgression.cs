...
namespace ChampionsOfForest
{
	public class EnemyProgression : MonoBehaviour
	{
		...
		private void Setup()
		{
			...
			//picking abilities
			if ((UnityEngine.Random.value < 0.1 || (_AI.creepy_boss && !_AI.girlFullyTransformed) || ModSettings.difficulty == ModSettings.Difficulty.Hell) && enemyType != Enemy.Baby)
			{
		...
		{
			if (_AI.creepy && _AI.pale)
			{
				enemyType = Enemy.PaleVags;
			}
			else if (_AI.creepy && !_AI.pale)
			{
				enemyType = Enemy.RegularVags;
			}
			else if (_AI.creepy_male && _AI.pale)
			{
			...
			else if (_AI.female && !_AI.pale && !_AI.painted)
			{
				enemyType = Enemy.NormalFemale;
			}
			else if (_AI.femaleSkinny && !_AI.pale && !_AI.painted)
			{
			...
			else if (_AI.male && !_AI.pale && !_AI.painted && !_AI.leader && !_AI.skinned)
			{
				enemyType = Enemy.NormalMale;
			}
			else if (_AI.maleSkinny && !_AI.pale && !_AI.painted && !_AI.leader && !_AI.skinned)
			{
